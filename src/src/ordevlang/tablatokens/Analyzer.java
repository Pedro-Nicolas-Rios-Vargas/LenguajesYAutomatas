...
public class Analyzer {
    ...
    public Analyzer(LinkedList<Token> tokensTable) {
        keyWords = LangKeyWords.keyWords;
    ...
    public LinkedList<Token> tokenizingLine(Line line) throws SyntaxException {
        ...
        for(int i = 0; i < length; i++) {
            ...
            if(numberFlag || exponentFlag) {
                ...
                if (numberFlag) {
                    if (Character.isDigit(actualChar)) {
                        ...
                        decimalFlag = true;
                        continue;
                    } else if(!(actualChar == 32 || LangKeyWords.isArithmeticOperator(actualChar))) {
                        if(actualChar == 46) {
                    ...
                    if (!expSignFlag) {
                        ...
                        if(i != code.length()-1)
                            continue;
                    } else if(!(actualChar == 32 || (LangKeyWords.isArithmeticOperator(actualChar) && numAfterExpSignFlag))) {
                        throw new SyntaxException(new Formatter().format("Error en la linea %d. Los numeros" +
                                " exponenciales no pueden contener \"%c\".", line.getLineNumber(), actualChar).toString());
                    }
                }

                if(actualChar == 32 || LangKeyWords.isArithmeticOperator(actualChar) || isLoopEnding) {
            ...
            if(sb.length() == 1) {
                ...
                } else if (LangKeyWords.isArithmeticOperator(single)) {
                    if (isActualEspace || isActualAlphabetic || isActualDigit) {
                        lexema = sb.toString();
    ...
    private void keyWordVariableTokens(Line line, LinkedList<Token> tokens, StringBuilder sb,
        ...
        if ((actualChar >= 32 && actualChar <= 34) // ESPACIO, !, "
                || (LangKeyWords.isArithmeticOperator(actualChar))   // OPERADORES
            ...
            if (isKeyWord(lexema)) {
                ...
                if(LangKeyWords.isDataType(lastTokenStr)) { // Verifying that the variable is declare
